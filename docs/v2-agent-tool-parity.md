...
| `get_asset_allocation` | READY | `API.AssetAllocation` first consumes household-scoped explicit current asset snapshots whose value is already in the household reporting currency, including explicit `equity`/`fund`/`gold`/`property` facts; an included linked snapshot suppresses that linked coarse ledger account to prevent double counting. Uncovered ledger accounts keep the conservative fallback: cash/checking/virtual → `cash`, savings/CD → `deposit`, and coarse investment/receivable/unknown assets → `other` with partial-quality warnings. Liabilities/hidden/container accounts and non-reporting-currency facts without an explicit reporting-currency valuation are omitted rather than inferred. Totals/shares remain delegated exactly once to `portfolio.Summarize`; valuation/FX freshness thresholds downgrade quality and `data_as_of` is the oldest included input. The Agent Adapter performs no portfolio recalculation. |
| `generate_monthly_report` | READY | `API.MonthlyReport(period)` maps `year`/`month` to `YYYY-MM`, reconstructs historical ledger balances from later transactions, and persists one immutable hashed artifact per household/period. Current planner-only historical fields are explicitly partial rather than presented as fully historical. |
...
The original seven are exercised by `TestAgentAdapterDeterministicParity`. Safe-to-spend and goal simulation are additionally exercised by `TestAgentAdapterPhaseOneDeterministicParity`. Extra debt payment is exercised by `TestAgentAdapterExtraDebtPaymentParity`. Spending analysis is exercised by `TestAgentAdapterSpendingAnalysisDeterministicParity`. Asset allocation is exercised by `TestAgentAdapterAssetAllocationDeterministicParity`, including explicit snapshot precedence and linked-account double-count prevention, while the adapter boundary test additionally proves server-injected household scope, strict empty input, metadata preservation, and exact typed business-payload passthrough. These parity suites compare direct `appapi.API` results with Agent Adapter business payloads for the same scoped household, inputs, clock, and deterministic fixtures rather than duplicating financial calculations in the adapter.
