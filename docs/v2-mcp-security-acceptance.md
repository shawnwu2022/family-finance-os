...
| Tool allowlist | MCP advertises exactly the eleven `READY` tools from `docs/v2-agent-tool-parity.md`; no unverified target tool is exposed |
...
| Real PostgreSQL MCP integration | CI creates a household, builds the secure handler, connects with the official Streamable HTTP client, discovers eleven tools, calls a scoped tool, and verifies a successful audit row |
...
- `openclaw mcp probe <server> --json` and verifies the selected server points at the expected HTTPS endpoint, exposes exactly the eleven release tools, and exposes no MCP resources/prompts;
